// Módulo principal con encapsulación y mejoras de rendimiento - VERSIÓN CONTABLE MAESTRA
(function() {
    // --- URLs de datos ---
    const urls = {
    ...
    // PARSER CONTABLE INVENCIBLE: Maneja decimales europeos/americanos y paréntesis negativos
    function parseNumber(val) {
        ...
        // Quitamos símbolos de moneda y letras, pero CONSERVAMOS comas y puntos
        str = str.replace(/[S/$\sa-zA-Z]/g, '');

        const lastComma = str.lastIndexOf(',');
        const lastDot = str.lastIndexOf('.');

        // Lógica de deducción de miles vs decimales
        if (lastComma > lastDot && lastComma !== -1 && lastDot !== -1) {
            str = str.replace(/\./g, '').replace(',', '.'); // Formato EU: 1.234,50
        } else if (lastDot > lastComma && lastDot !== -1 && lastComma !== -1) {
            str = str.replace(/,/g, ''); // Formato US: 1,234.50
        } else if (lastComma !== -1 && lastDot === -1) {
            let parts = str.split(',');
            if (parts[parts.length - 1].length <= 2) {
                str = str.replace(',', '.'); // Formato mixto corto: 15,50
            } else {
                str = str.replace(/,/g, ''); // Miles sin decimales: 1,234
            }
        }
    ...
    function findColumn(obj, options) {
        ...
        for (let opt of options) {
            const normOpt = normalizeText(opt); const found = keys.find(k => normalizeText(k) === normOpt);
            if (found) return found;
        }
        for (let opt of options) {
            const normOpt = normalizeText(opt); const found = keys.find(k => normalizeText(k).includes(normOpt));
            if (found) return found;
        }
        return null;
    }
    ...
    function initColumns() {
        if (data.vendedoresRaw.length) {
            cols.vendedores = { id: findColumn(data.vendedoresRaw[0], ['ID_VENDEDOR']), nombre: findColumn(data.vendedoresRaw[0], ['NOMBRE']), apellido: findColumn(data.vendedoresRaw[0], ['APELLIDO']), meta: findColumn(data.vendedoresRaw[0], ['META']), tipo: findColumn(data.vendedoresRaw[0], ['TIPO']) };
        }
        if (data.ventasRaw.length) {
            cols.ventas = {
                ...
                total: findColumn(data.ventasRaw[0], ['PRECIO TOTAL', 'TOTAL', 'IMPORTE LINEA']),
                fecha: findColumn(data.ventasRaw[0], ['FECHA DE VENTA', 'FECHA']),
                documento: findColumn(data.ventasRaw[0], ['Documento_Numero']),
                razon: findColumn(data.ventasRaw[0], ['RAZÓN SOCIAL']),
                // NUEVA COLUMNA DETECTADA
                tipo: findColumn(data.ventasRaw[0], ['TIPO DE VENTA', 'TIPO_VENTA']) 
            };
        }
        if (data.productosRaw.length) {
            cols.productos = { idVendedor: findColumn(data.productosRaw[0], ['ID_VENDEDOR']), idCliente: findColumn(data.productosRaw[0], ['ID_CLIENTE']), documento: findColumn(data.productosRaw[0], ['Documento_Numero']), producto: findColumn(data.productosRaw[0], ['NOMBRE DEL PRODUCTO', 'PRODUCTO']), unid: findColumn(data.productosRaw[0], ['CANTIDAD UNID']), caja: findColumn(data.productosRaw[0], ['CANTIDAD CAJA']), fecha: findColumn(data.productosRaw[0], ['FECHA DE VENTA']) };
        }
        if (data.clientesRaw.length) {
            cols.clientes = { id: findColumn(data.clientesRaw[0], ['ID_CLIENTE']), documento: findColumn(data.clientesRaw[0], ['Documento_Numero']), razon: findColumn(data.clientesRaw[0], ['RAZÓN SOCIAL']), ubicacion: findColumn(data.clientesRaw[0], ['UBICACIÓN']), idVendedor: findColumn(data.clientesRaw[0], ['ID_VENDEDOR']), estado: findColumn(data.clientesRaw[0], ['ESTADO DE VENTA']) };
        }
    }
    ...
    function normalizeAllData() {
        ...
        for (const row of data.ventasRaw) {
            ...
            // REGLA DE RESTA: Si es Nota de Crédito o Anulación, restarlo automáticamente.
            if (cols.ventas.tipo) {
                const tipoVenta = normalizeText(row[cols.ventas.tipo]);
                if (tipoVenta.includes('NOTA') || tipoVenta.includes('CREDITO') || tipoVenta.includes('NC') || tipoVenta.includes('DEVOL') || tipoVenta.includes('ANULAD')) {
            ...
            const idVendedor = normalizeText(row[cols.ventas.idVendedor]); let idCliente = normalizeText(row[cols.ventas.idCliente]);
            ...
            if (idVendedor) { if (!ventasPorVendedor.has(idVendedor)) ventasPorVendedor.set(idVendedor, []); ventasPorVendedor.get(idVendedor).push(ventaNorm); }
            if (idCliente) { if (!ventasPorCliente.has(idCliente)) ventasPorCliente.set(idCliente, []); ventasPorCliente.get(idCliente).push(ventaNorm); }
            if (fechaObj && fechaObj.fullDate) { if (!maxDate || fechaObj.fullDate > maxDate) maxDate = fechaObj.fullDate; }
        }

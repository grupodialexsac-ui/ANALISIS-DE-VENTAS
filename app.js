...
function cargarVistaGlobal(liElement) {
    ...
    graficos.rankingMeta = new Chart(document.getElementById('chartRankingMeta').getContext('2d'), { 
        type: 'bar', 
        data: { labels: rankingMetaArr.map(r => r.nombre), datasets: [{ label: 'Avance de la Meta', data: dataLogrado, backgroundColor: '#4285f4' }, { label: 'Meta Total (Faltante)', data: dataFaltante, backgroundColor: '#ea4335' }] }, 
...
function cargarVistaVendedor(vendedorData, liElement) {
    ...
    document.getElementById('cardVendedorProdUnid').style.display = 'flex'; document.getElementById('cardVendedorProdCaja').style.display = 'flex'; document.getElementById('cardVendedorClientes').style.display = 'flex';
    ...
    let pColId = getCol(db.productos[0], 'VENDEDOR'); let pColNombre = getCol(db.productos[0], 'NOMBRE');
    let pColCaja = getCol(db.productos[0], 'CAJA'); let pColUnid = getCol(db.productos[0], 'UNID');
...
function ejecutarBusquedaCliente() {
    ...
    document.getElementById('kpiBusqueda').innerHTML = `
        <div class="kpi-box destacado" style="max-width:300px;">
...
function dibujarLinea(dailyData, canvasId, labelText, containerObj, keyObj) {
    ...
    containerObj[keyObj] = new Chart(document.getElementById(canvasId).getContext('2d'), { 
        type: 'line', 
        data: { labels: fechasOrdenadas.map(f => f.split('/')[0]+'/'+f.split('/')[1]), datasets: [{ label: labelText, data: fechasOrdenadas.map(f => dailyData[f]), borderColor: '#4285f4', fill: false, tension: 0.1 }] },
